...
/**
 * v1.3.0 → v1.4.0: Deep link support for web dashboard
 * - Write CLAUDE.md.new with [REQUEST_WEB_FILE:path] marker docs
 * - Clear session store
 */
module.exports = function migrate(ctx) {
  ...
  if (fs.existsSync(newClaudeMdTemplate) && fs.existsSync(claudeMdPath)) {
    ...
    if (!existing.includes('REQUEST_WEB_FILE')) {
      ...
      log.info(`Workspace ${wsId}: CLAUDE.md.new written (v1.4.0 deep link)`);
    }
  }
  ...
  if (fs.existsSync(sessionFile)) {
    ...
    log.info(`Workspace ${wsId}: session store cleared (v1.4.0)`);
  }
};
