...
사용자 활동에 기반하여 발동하는 이벤트 기반 작업 — cron 타이머가 아닙니다:
...
├── CLAUDE.md      # 에이전트 지시 (행동, 안전 규칙)
├── USER.md        # 사용자 정보 (이름, 언어, 관심사, 시간대)
├── SOUL.md        # 에이전트 성격과 가치관
├── IDENTITY.md    # 에이전트 이름, 이모지, 분위기
├── SEEDS.md       # 지식 시드 — 탐색할 주제
├── MEMORY.md      # 장기 큐레이션 기억
├── memory/        # 일일 노트 (YYYY-MM-DD.md)
└── jobs.json      # 사용자 커스텀 작업
```
